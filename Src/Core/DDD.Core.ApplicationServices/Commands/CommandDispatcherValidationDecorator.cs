...
public class CommandDispatcherValidationDecorator : CommandDispatcherDecorator
{
    ...
    public override async Task<CommandResult> Send<TCommand>(TCommand command)
    {
        ...
        {
            _logger.LogInformation("Validating command of type {CommandType} With value {Command}  failed. Validation errors are: {ValidationErrors}", command.GetType(), command, validationResult.Messages);
